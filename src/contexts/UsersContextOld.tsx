...
interface UsersContextType {
  ...
  approveWriterApplication: (writerId: string, reviewedBy: string, notes?: string) => void;
  rejectWriterApplication: (writerId: string, reason: string, reviewedBy: string, notes?: string) => void;
  submitWriterApplication: (userId: string, applicationData: Partial<Writer>) => void;
}
...
export function UsersProvider({ children }: { children: React.ReactNode }) {
  ...
  // New approval functions
  const approveWriterApplication = useCallback((writerId: string, reviewedBy: string, notes?: string) => {
    setWriters(prev => prev.map(writer => 
      writer.id === writerId 
        ? { 
            ...writer, 
    ...
    logActivity(writerId, 'profile_updated', 'Writer application approved', 
      { reviewedBy, notes });
  }, [logActivity]);

  const rejectWriterApplication = useCallback((writerId: string, reason: string, reviewedBy: string, notes?: string) => {
    setWriters(prev => prev.map(writer => 
      writer.id === writerId 
        ? { 
            ...writer, 
    ...
    logActivity(writerId, 'profile_updated', `Writer application rejected: ${reason}`, 
      { reviewedBy, reason, notes });
  }, [logActivity]);

  const submitWriterApplication = useCallback((userId: string, applicationData: Partial<Writer>) => {
    ...
    if (existingWriter) {
      ...
      logActivity(existingWriter.id, 'profile_updated', 'Writer application submitted', 
        { applicationData });
    } else {
      // Create new writer with application data
      const newWriter = createWriter({
        ...applicationData,
        status: 'application_submitted',
        applicationSubmittedAt: new Date().toISOString()
      });
      
      logActivity(newWriter.id, 'profile_updated', 'New writer application submitted', 
        { applicationData });
    }
  }, [writers, createWriter, logActivity]);
